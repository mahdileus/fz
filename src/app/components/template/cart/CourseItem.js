...
export default function CourseItem({ course, onRemove }) {
  ...
  // بررسی وجود course
  if (!course) {
    return <div className="text-center text-red-500">داده دوره نامعتبر است</div>;
  }
  ...
  {course.price !== undefined ? (
    course.discountedPrice ? (
      <>
        ...
        <span className="text-primary">
          {course.discountedPrice.toLocaleString()} تومان
        </span>
      </>
    ) : (
      <span className="text-primary">
    ...
    <span className="text-red-500">قیمت نامشخص</span>
  )}
</p>
